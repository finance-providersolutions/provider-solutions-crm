...
import {
  computeOpportunityProjection,
  projectionGuardOn,
  deriveStaffing,
  staffingDiagnostic,
} from './projectionShared';

// V6 — "Can't Staff": surfaces open + filled opportunities that have
// ZERO ready providers (no one in the privileged / applied / selected
// / eligible tiers after eligibilityFilter + classifyProviderStage).
//
// Contrast with V5: V5 is value-ranked ("biggest deals"); V6 is a
// constraint view ("biggest deals you can't actually staff"). Same
// underlying matching loop V4 already runs per-open-opp at lines
// 169–197; just iterated across open AND filled stages and re-
// presented around the unfillable-flag.
//
// Honesty rule: V6 keeps UNMODELED opps in the loop (no rates set).
// The staffing gap is real regardless of rate-modeling state — a
// brand-new opp without rates can still have zero ready providers.
// Unmodeled rows render with "—/yr GP" on the financial side and
// the staffing diagnostic carries the row. Different exclusion
// from V5 / the /financial-projections page (both exclude unmodeled
// from their rankings); this is what makes "Can't Staff" complete.
//
// V1–V5 untouched.
...
export default function HomeV6() {
  ...
  // ── Can't Staff (V6) ──
  // Single useMemo over opportunities × matching data: for each
  // OPEN or FILLED opp, compute staffing tier counts via
  // eligibilityFilter + classifyProviderStage (same loop V4 runs
  // per-open-opp), surface those with readyCount === 0. Includes
  // unmodeled opps — staffing gap is independent of rate state.
  const cantStaff = useMemo(() => {
    const ready =
      !opportunities.loading &&
      !providers.loading &&
      !cred.loading &&
      !placements.loading;
    if (!ready) return { ready: false, top5: [], flaggedCount: 0, openCount: 0 };

    const flagged = [];
    let openCount = 0;
    for (const opp of opportunities.data) {
      const isOpen   = ACTIVE_OPPORTUNITY_STAGES.includes(opp.stage);
      const isFilled = opp.stage === 'filled';
      if (!isOpen && !isFilled) continue;
      if (isOpen) openCount += 1;

      const staffing = deriveStaffing({
        opp,
        providers: providers.data,
        licensesByProvider:    cred.licensesByProvider,
        credentialsByProvider: cred.credentialsByProvider,
        privilegesByProvider:  cred.privilegesByProvider,
        placementsByOpportunity: placements.byOpportunity,
      });
      if (staffing.readyCount > 0) continue; // not an "unfillable" row

      // Row record — projection slice if modeled, null fields if
      // not. Diagnostic sub-line names the gap type.
      const modeled = !projectionGuardOn(opp);
      const proj = modeled ? computeOpportunityProjection(opp) : null;
      flagged.push({
        opp,
        ...(proj ?? {}),
        modeled,
        staffing,
        diagnostic: staffingDiagnostic({
          opp,
          providers: providers.data,
          licensesByProvider: cred.licensesByProvider,
          staffing,
    ...
    // Order: open before filled (operational urgency); within each,
    // modeled opps before unmodeled (projection value visible);
    // within those, annualGP descending (most valuable first).
    flagged.sort((a, b) => {
      if (a.isOpen !== b.isOpen)     return a.isOpen ? -1 : 1;
      if (a.modeled !== b.modeled)   return a.modeled ? -1 : 1;
      return (b.annualGP ?? 0) - (a.annualGP ?? 0);
    });

    return {
      ready: true,
      top5:         flagged.slice(0, 5),
      flaggedCount: flagged.length,
      openCount,
    };
  }, [opportunities.data, opportunities.loading, providers.data, providers.loading, cred, placements]);
  ...
  return (
    <>
      ...
      {/* ── CAN'T STAFF (V6 — opportunity readiness lens) ── */}
      <SectionHeader text="Can't Staff" first />
      <BBox className="mb-10">
        <div className="font-mono text-[10px] uppercase tracking-[0.12em] text-text-dim text-center mb-4">
          Open + filled opps with no ready provider
        </div>
        {!cantStaff.ready && (
          <div className="font-mono text-[11px] uppercase tracking-[0.12em] text-text-dim text-center py-3">Loading…</div>
        )}
        {cantStaff.ready && cantStaff.top5.length === 0 && (
          <div className="font-mono text-[11px] uppercase tracking-[0.12em] text-text-dim text-center py-3">
            {cantStaff.openCount === 0
              ? 'No open opportunities.'
              : 'Every active opp has at least one ready candidate.'}
          </div>
        )}
        {cantStaff.ready && cantStaff.top5.length > 0 && (
          <ul className="divide-y divide-border/40">
            {cantStaff.top5.map(row => (
              <li key={row.opp.id}>
                <ProjectionRow
                  opp={row.opp}
                  annualGP={row.modeled ? row.annualGP : null}
                  perShiftGP={row.perShiftGP}
                  perShiftMargin={row.perShiftMargin}
                  targetShiftsPerYear={row.targetShiftsPerYear}
                  stageBadge
                  secondary="shifts"
                  staffingDiagnostic={row.diagnostic}
                  staffingTone="danger"
                />
              </li>
            ))}
        ...
        <div className="mt-5 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.12em] text-text-dim">
          <span>
            {cantStaff.flaggedCount} flagged of {cantStaff.openCount} open
          </span>
