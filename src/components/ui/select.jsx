...
const SelectContent = React.forwardRef(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        // z-[210] sits above the suite's fixed chrome (PageHeader z-200,
        // page bars z-150) so the open options list is never hidden
        // behind the sticky header — including the collision-flip case
        // where Radix opens the popper upward into the header band (a
        // short/zoomed viewport with the trigger near the top, e.g. the
        // credential form's first field). Default popper placement still
        // opens downward and only flips when there is genuinely no room.
        "relative z-[210] max-h-[--radix-select-content-available-height] min-w-[8rem] overflow-y-auto overflow-x-hidden rounded-md border bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-[--radix-select-content-transform-origin]",
        position === "popper" &&
