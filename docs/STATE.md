...
Phase 1 is shipped. Phase 2 is in flight, partially landed in the working tree: the schema migration, the AppSheet import script, and the image-upload plumbing exist; the opportunities, providers, and tasks pages, the GP modeler, and the updated Home dashboard do not yet. Phase 3 (credentialing) and Phase 4 (matching) are further out. See ROADMAP.md for the precise split between done and remaining.
...
components/ui/ is the shadcn primitives folder: avatar, badge, button, dialog, dropdown-menu, input, label, progress, select, sonner, table, tabs, textarea. Avatar and progress were added in Phase 2 to support image upload UI.

Feature-scoped components: components/organizations/OrganizationFormDialog.jsx, components/contacts/ContactFormDialog.jsx, components/activities/ActivityFeed.jsx and LogActivityForm.jsx, components/uploads/ImageUpload.jsx (drag-drop + progress + size/type validation, written so it extends to the Phase 3 credentials bucket later) and components/uploads/Thumb.jsx (thumbnail with initials fallback).
